...
contract TestOtterSwapRouter is IOtterSwapSwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IOtterSwapPool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function otterswapSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IOtterSwapPool(msg.sender).token0()
                : IOtterSwapPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IOtterSwapPool(pools[0]).token1();
            IOtterSwapPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IOtterSwapPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IOtterSwapPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
