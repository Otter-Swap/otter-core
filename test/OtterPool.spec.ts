...
import { TestOtterSwapSwapPay } from '../typechain/TestOtterSwapSwapPay'
...
import { TestOtterSwapReentrantCallee } from '../typechain/TestOtterSwapReentrantCallee'
...
describe('OtterSwapPool', () => {
  ...
  let factory: OtterSwapFactory
  let pool: MockTimeOtterSwapPool

  let swapTarget: TestOtterSwapCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeOtterSwapPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestOtterSwapReentrantCallee')
      ).deploy()) as TestOtterSwapReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestOtterSwapSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestOtterSwapSwapPay')
      underpay = (await underpayFactory.deploy()) as TestOtterSwapSwapPay
