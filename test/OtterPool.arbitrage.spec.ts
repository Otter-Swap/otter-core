...
import { OtterSwapPoolSwapTest } from '../typechain/OtterSwapPoolSwapTest'
...
describe('OtterSwapPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('OtterSwapPoolSwapTest')
            const tester = (await testerFactory.deploy()) as OtterSwapPoolSwapTest
          ...
          let tester: OtterSwapPoolSwapTest
